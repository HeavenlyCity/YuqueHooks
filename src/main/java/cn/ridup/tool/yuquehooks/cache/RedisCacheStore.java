...
public class RedisCacheStore extends AbstractStringCacheStore {
    ...
    private static final String REDIS_PREFIX = "yuque.redis.";
    ...
    void putInternal(@NonNull String key, @NonNull CacheWrapper<String> cacheWrapper) {
        ...
        if (cacheWrapper.getExpireAt() != null) {
            long expire = cacheWrapper.getExpireAt()
                .getTime() - System.currentTimeMillis();
            redisTemplate.opsForValue()
                .set(REDIS_PREFIX + key, cacheWrapper.getData(), expire, TimeUnit.MILLISECONDS);
        } else {
            redisTemplate.opsForValue()
                .set(REDIS_PREFIX + key, cacheWrapper.getData());
        }
    ...
    public LinkedHashMap<String, String> toMap() {
        ...
        keys.forEach(key -> map.put(key, redisTemplate.opsForValue()
            .get(key)));
        return map;
    }
